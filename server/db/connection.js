...
function getDb() {
  if (!db) {
    db = new sqlite3.Database(
      ...
      // Enable foreign key constraint enforcement (cascading deletes)
      db.run('PRAGMA foreign_keys = ON', (err) => {
        if (err) console.warn('⚠ Foreign keys error:', err.message);
      });
      // Rollback-Journal (DELETE) statt WAL.
      // GRUND (SQLITE_CORRUPT-Dauerfix): Teilen sich ZWEI Prozesse (App + Admin-Panel,
      // gleiches Image) dieselbe turmstatus.db auf einem geteilten Volume, ist WALs
      // Shared-Memory-Koordination (`-shm`, mmap) prozessübergreifend NICHT kohärent →
      // „database disk image is malformed" schon beim ersten gleichzeitigen Schreiben.
      ...
      // Der zentrale Fix ist EIN Prozess für beide Ports (server.js bettet das
      // Admin-Panel ein) – DELETE ist die zweite Sicherungslinie.
