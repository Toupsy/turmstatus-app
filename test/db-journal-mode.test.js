...
// WAL ist zwischen zwei Prozessen (App + Admin-Panel) auf einem geteilten Volume
// nicht prozess-kohärent → SQLITE_CORRUPT beim Schreiben (z. B. audit_log).
...
function journalModeAfterInit(dbPath) {
  const script = `
    ...
    const { dbAll, dbRun } = require('./server/db/connection');
    (async () => {
      await initDatabase();
